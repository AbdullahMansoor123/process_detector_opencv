...
def process_indicator(csv_path, video_path):
    ...
    while True:
        ...
        for row in csv_df.itertuples():  # read each row by row
            ...
            if count in range(start_frame+2, stop_frame+1):
                cv2.putText(frame, f'Process: {process}', (int(frame_width/2)-150, 25), 2, 1, (0, 255, 255), 2)
                cv2.putText(frame, f'Start Frame: {start}', (int(frame_width/2)-150, 50), 2, 1, (0, 255, 0), 2)

            elif count in range(stop_frame+1,stop_frame+3):
                cv2.putText(frame, f'Process: {process}', (int(frame_width/2)-150, 25), 2, 1, (0, 255, 255), 2)
                cv2.putText(frame, f'Start Frame: {start}', (int(frame_width/2)-150, 50), 2, 1, (0, 255, 0), 2)
                cv2.putText(frame, f'End Frame: {stop}', (int(frame_width/2)-150, 75), 2, 1, (0, 0, 255), 2)
...
vid_files = glob.glob(data_dir + '*.mp4')
